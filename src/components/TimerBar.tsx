  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3 w-full">
      ...
      <input
        ...
        onKeyDown={(e) => { if (e.key === 'Enter' && !isRunning) startTimer(); }}
      />

      {/* Projekt picker */}
      <div className="relative hidden sm:block" ref={projectPickerRef}>
        <button
          onClick={() => { setShowProjectPicker(!showProjectPicker); setShowTaskPicker(false); }}
        ...
        {showProjectPicker && (
          <div
            className="absolute top-full right-0 mt-1 w-56 rounded-lg border shadow-lg z-50 py-1 max-h-60 overflow-y-auto"
          ...
          >
            ...
            {projects.length === 0 && (
              <div className="px-3 py-2 text-xs" style={{ color: 'var(--text-muted)' }}>Vytvořte projekt v sekci Projekty.</div>
            )}
          </div>
        )}
      </div>
      ...
      <div className="relative hidden sm:block" ref={taskPickerRef}>
        <button
          onClick={() => { setShowTaskPicker(!showTaskPicker); setShowProjectPicker(false); }}
        ...
        {showTaskPicker && (
          <div
            className="absolute top-full right-0 mt-1 w-64 rounded-lg border shadow-lg z-50 py-1 max-h-72 overflow-y-auto"
            style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}
          >
            {/* Kategorie */}
            {categories.length > 0 && (
      ...
      {/* Čas */}
      <div
        className="text-lg sm:text-xl font-mono font-bold tabular-nums min-w-[85px] sm:min-w-[100px] text-center"
