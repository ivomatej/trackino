...
<ul>
  <li><strong>Refaktoring: Fakturace</strong>:
    <ul>
      <li>Původní monolitický soubor <code>invoices/page.tsx</code> (794 ř.) rozdělen na 3 nové soubory v adresáři <code>components/</code></li>
      <li><code>useInvoices.ts</code> – custom hook se veškerým stavem, fetch logikou, výpočetními hodnotami a CRUD akcemi (~370 ř.)</li>
      <li><code>CurrentPeriodCard.tsx</code> – komponenta zobrazující aktuální fakturu za fakturační období (stav, částka, datum, PDF)</li>
      <li><code>InvoicesContent.tsx</code> – orchestrátor skládající celou stránku (header, bannery, formulář, filtry, taby, modaly)</li>
      <li><code>page.tsx</code> redukován na ~13 řádků (auth guard + WorkspaceProvider)</li>
    </ul>
  </li>
</ul>
