...
<ul>
  ...
  <li><strong>Složky a sdílení</strong> – až 5 úrovní podsložek; sdílení s celým workspacem nebo konkrétními uživateli; <strong>menu tří teček u složky</strong> – na desktopu se zobrazí při najetí myší, na mobilu je vždy viditelné; nabídka obsahuje: Přidat podsložku, Posunout nahoru/dolů (jen při ručním řazení), Sdílet, Přejmenovat, Smazat; název aktivní složky se zobrazuje v záhlaví pravého panelu; <strong>smazání složky</strong> automaticky přesune všechny poznámky (včetně podsložek) do Archivu (žádná poznámka se neztratí)</li>
  <li><strong>Rich text editor</strong> – formátování B/I/U, odrážkový a číselný seznam (odrážky/čísla jsou správně viditelná v editoru), kódový blok (tlačítko <code>&lt;/&gt;</code> vloží blok s copy ikonkou vpravo nahoře); URL jsou automaticky klikatelné; <strong>vkládání textu z jiných nástrojů</strong> – podbarvení pozadí se automaticky odstraní, vloží se pouze text bez nežádoucích barev pozadí</li>
  ...
  <li><strong>Příznaky</strong> – Oblíbená (hvězdička), Důležitá (červená); lze nastavit přes toolbar nebo ikonky v záhlaví poznámky</li>
  <li><strong>Stav Hotovo</strong> – pokud je v Nastavení zapnutý toggle <em>Stav „Hotovo"</em>: v toolbaru editoru se zobrazí zelené pill tlačítko <em>Hotovo</em>; hotové poznámky jsou ve výpisu zobrazeny s přeškrtnutým názvem, sníženou průhledností (0,45) a zelenou fajfkou; ve výpisu je také přímé tlačítko přepnutí stavu; stav se synchronizuje s databází; v záhlaví výpisu se zobrazí tlačítko <strong>Skrýt hotové</strong> – po kliknutí skryje všechny poznámky označené jako hotové (aktivní stav je zvýrazněn primární barvou, opětovným kliknutím se zobrazí zpět)</li>
  <li><strong>Přesunout do složky</strong> – v detailu poznámky je v záhlaví ikonka složky pro přesun; ve výpisu je ikonka šipky dolů</li>
  <li><strong>Nová poznámka</strong> – po vytvoření se automaticky zobrazí kurzor v poli názvu; stiskem Enter přejde kurzor do editoru</li>
  <li><strong>Archiv</strong> – archivované poznámky jsou v samostatné sekci; archivovat/obnovit lze přes ikonky ve výpisu</li>
...
<ul>
  ...
  <li><strong>4 záložky</strong> – Předplatná (hlavní výpis), Tipy (doporučení), Kategorie (správa kategorií s barvami a podkategoriemi), Přístupy (evidence přístupů uživatelů ke službám)</li>
  ...
  <li><strong>Podkategorie</strong> – kategorie mohou mít podkategorie (např. Marketing → SEO, PPC). Při filtrování dle nadřazené kategorie se automaticky zahrnují i podkategorie. Smazání nadřazené kategorie smaže i podkategorie.</li>
  <li><strong>Filtrování a řazení</strong> – fulltextové hledání, filtr dle stavu/typu/kategorie (včetně podkategorií), řazení dle názvu/ceny/další platby/stavu/hodnocení</li>
