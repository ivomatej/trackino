...
const PAGE_SLUG = 'office-rules';

const DEFAULT_CONTENT = `
<h2>Pravidla v kanceláři</h2>
<p>Zde správce workspace může zapsat pravidla pro provoz kanceláře, etiketu a každodenní chod týmu.</p>
<p><em>Obsah upraví správce (Admin/Owner) kliknutím na tlačítko Upravit.</em></p>
`;

// ─── Interní komponenta ───────────────────────────────────────────────────────

function OfficeRulesContent() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (!wsLoading && !hasModule('office_rules')) router.replace('/');
  ...
  if (loading) {
    return (
      <DashboardLayout moduleName="Pravidla v kanceláři">
...
export default function OfficeRulesPage() {
